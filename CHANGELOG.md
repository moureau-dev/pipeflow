...
- **Tool-mode encodings** — `LLMRequest.toolMode` (and the matching adapter option, which the request overrides) selects how tool calls are encoded on the wire: `native` (provider `tools`/`tool_calls`, default), `envelope` (`response_format` JSON envelope translated back into `tool_call` events — for endpoints with structured outputs but no native tool calling), or `prompted` (the same envelope requested by instruction — the universal fallback). Every mode yields the same event surface, so agents and the orchestrator never change.
- **ToolModeBenchmark** — a diagnostic class (exported from `@moureau/pipeflow/providers`) that benchmarks the three tool modes through the real adapter path, reporting per-mode availability, p50/p90/p99 decision latency, and median cost from OpenRouter registry pricing, plus the fastest/cheapest working modes (`bun scripts/envelope-vs-native.ts` wraps it as a CLI; `scripts/tool-envelope-probe.ts` gates a model's envelope validity when onboarding).
